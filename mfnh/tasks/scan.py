...
def _scan_root(sess, root: Root):
    ...
    for item in _filter_out_known_files(sess, root, scanned_movies):
        ...
        if kind == _FILE_TYPE_MOVIE:
            ...
            movie_file = File(path=str_rel_path(scanned_movie.path, root_path), root=root)
            db_movie.file = movie_file

            if scanned_movie.backdrop:
                img_file = File(path=str_rel_path(scanned_movie.backdrop, root_path), root=root)
                db_movie.backdrop = img_file
                sess.add(img_file)

            if scanned_movie.poster:
                img_file = File(path=str_rel_path(scanned_movie.poster, root_path), root=root)
                db_movie.poster = img_file
                sess.add(img_file)

            for (sub_path, lang) in scanned_movie.subs:
                sub_file = File(path=str_rel_path(sub_path, root_path), root=root)
